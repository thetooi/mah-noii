...
function show_level(){
        ...
        $query = mysqli_query($GLOBALS["___mysqli_ston"],  'SELECT * FROM reputationlevel ORDER BY minimumreputation ASC' );
if( ! mysqli_num_rows( $query ) ){
        ...
        $html .= "<form action='reputation_ad.php' name='show_rep_form' method='post'><input name='mode' value='doupdate' type='hidden' />";
...
if( $type == 'edit' ){
            $query = mysqli_query($GLOBALS["___mysqli_ston"],  'SELECT * FROM reputationlevel WHERE reputationlevelid='.intval($input['reputationlevelid']) ) or sqlerr(__LINE__,__FILE__);
...
if( $type == 'new' ){
            @mysqli_query($GLOBALS["___mysqli_ston"],  "INSERT INTO reputationlevel ( minimumreputation, level )
                            VALUES  ($minrep, $level )" );
        }
        elseif( $type == 'edit' )
        {
            ...
            $query = mysqli_query($GLOBALS["___mysqli_ston"],  "SELECT reputationlevelid FROM reputationlevel WHERE
            ...
            @mysqli_query($GLOBALS["___mysqli_ston"],  "UPDATE reputationlevel SET minimumreputation = $minrep, level = $level
        ...
        else
        {
            ...
            if( is_array($ids) && count($ids) )
            {
                foreach( $ids as $k => $v )
                {
                    @mysqli_query($GLOBALS["___mysqli_ston"],  "UPDATE reputationlevel SET minimumreputation = ".intval($v)." WHERE reputationlevelid = ".intval($k) );
                }
            }
            else
            {
...
if( ! isset($input['reputationlevelid']) || ! is_valid_id($input['reputationlevelid']) )
        ...
        $query = mysqli_query($GLOBALS["___mysqli_ston"],  "SELECT reputationlevelid FROM reputationlevel WHERE reputationlevelid=$levelid" );
        ...
        @mysqli_query($GLOBALS["___mysqli_ston"],  "DELETE FROM reputationlevel WHERE reputationlevelid=$levelid" );
        rep_cache();
...
if( ! isset($input['reputationid']) || ! is_valid_id($input['reputationid']) )
        ...
        $query = mysqli_query($GLOBALS["___mysqli_ston"],  "SELECT r.*, p.topicid, t.subject, leftfor.username as leftfor_name,
...
global $now_date, $time_offset, $input;
        ...
        if( isset($input['dolist']) ){
            ...
            {
                $left_b = @mysqli_query($GLOBALS["___mysqli_ston"],  "SELECT id FROM users WHERE username = ".sqlesc($input['leftby']) );
            ...
            {
                $left_f = @mysqli_query($GLOBALS["___mysqli_ston"],  "SELECT id FROM users WHERE username = ".sqlesc($input['leftfor']) );
            ...
            $query = mysqli_query($GLOBALS["___mysqli_ston"],  "SELECT COUNT(*) AS cnt FROM reputation r WHERE $cond" );
            //print_r($input); exit;
            ...
            // mofo query!
            $query = mysqli_query($GLOBALS["___mysqli_ston"],  "SELECT r.*, p.topicid, leftfor.id as leftfor_id,
...
function do_delete_rep()
    {
        ...
        $query = mysqli_query($GLOBALS["___mysqli_ston"],  "SELECT reputationid, reputation, userid FROM reputation WHERE reputationid=".intval($input['reputationid'] ) );
        ...
        // do the delete
        @mysqli_query($GLOBALS["___mysqli_ston"],  "DELETE FROM reputation WHERE reputationid=".intval($r['reputationid'] ) );
        @mysqli_query($GLOBALS["___mysqli_ston"],  "UPDATE users SET reputation = (reputation-{$r['reputation']} ) WHERE id=".intval($r['userid']) );
...
function do_edit_rep()
    {
        ...
        // valid ID?
        $query = mysqli_query($GLOBALS["___mysqli_ston"],  "SELECT reputationid, reason, userid FROM reputation WHERE reputationid=".intval($input['reputationid'] ) );
    ...
    if( $r['reason'] != $reason || $oldrep != $newrep)
        {
        @mysqli_query($GLOBALS["___mysqli_ston"],  "UPDATE reputation SET reputation = ".intval($newrep).", reason = ".sqlesc($reason)." WHERE reputationid = ".intval($r['reputationid']) );
        }
        ...
        $diff = $oldrep - $newrep;
        @mysqli_query($GLOBALS["___mysqli_ston"],  "UPDATE users SET reputation = (reputation-{$diff}) WHERE id=".intval($r['userid']) );
...
function redirect($url, $text, $time=2)
    {
        ...
        $html = "<meta http-equiv='refresh' content=\"{$time}; url={$FMED['baseurl']}/{$url}\">
                            <div>
                            <div>Redirecting</div>
                            <div style='padding:8px'>
                             ...
                             <br />
                             <br />
                             <center><a href='{$FMED['baseurl']}/{$url}'>Click here if not redirected...</a></center>
                             </div>
                            </div>
                           </div>";
...
function rep_cache()
    {
        ...
        $query = @mysqli_query($GLOBALS["___mysqli_ston"],  "SELECT * FROM reputationlevel" );
