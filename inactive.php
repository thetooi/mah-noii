...
<?
echo("<h2>".htmlspecialchars($count)." accounts inactive for longer than ".htmlspecialchars($days)." days.</h2>");
echo("<form action=\"inactive.php\" method=\"post\">");
echo("<table class=main border=1 cellspacing=0 cellpadding=5><tr>\n");
echo("<td class=colhead>Username</td>");
echo("<td class=colhead>Class</td>");
echo("<td class=colhead>Mail</td>");
echo("<td class=colhead>Ratio</td>");
echo("<td class=colhead>Last Seen</td>");
echo("<td class=colhead align=\"center\">x</td>");
...
echo("<tr>");
echo("<td><a href=\"userdetails.php?id=".$arr["id"]."\">".$arr["username"]."</a></td>");
echo("<td>".$class."</td>");
echo("<td><a href=\"mailto:".$arr["email"]."\">".htmlspecialchars($arr["email"])."</a></td>");
echo("<td>".$ratio."</td>");
echo("<td>".$last_seen."</td>");
echo("<td align=\"center\" bgcolor=\"#FF0000\"><input type=\"checkbox\" name=\"userid[]\" value=\"".$arr["id"]."\" /></td>");
echo("</tr>");
}
echo("<tr><td colspan=\"6\" class=\"colhead\" align=\"center\">
<select name=\"action\">
...
if ($date["last_mail"] > 0 )
echo("<tr><td colspan=\"6\" class=\"colhead\" align=\"center\" style=\"color:red;\">Last mail sent by <a href=\"usersdetails.php?id=".htmlspecialchars($date["userid"])."\">".htmlspecialchars($date["username"])."</a> on <b>".gmdate("d M Y",$date["last_mail"])."</b> and <b>".$date["mails"]."</b> mail".($date["mails"] > 1 ? "s" : "")." was sent!</td></tr>");
}

echo("</table></form>");
}else{
echo("<h2>No account inactive for longer than ".$days." days.</h2>");
}
stdfoot();
?>
