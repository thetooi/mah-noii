...
*/
	...
	*/
	$forum_pics = array('default_avatar' => 'default_avatar.gif', 	'arrow_up' 		=> 'p_up.gif', 		'online_btn' 		=> 'button_online.gif',
						'offline_btn' 	 => 'button_offline.gif', 	'pm_btn' 		=> 'pm.gif', 		'p_report_btn' 	 	=> 'report.gif',
...
function forum_stats()
{
	...
	$res = mysql_query("SELECT id, username, donor, warned, class FROM users WHERE forum_access >= ".sqlesc(get_date_time(gmtime() - 180))." ORDER BY forum_access DESC") or sqlerr(__FILE__, __LINE__);
	while ($arr = mysql_fetch_assoc($res))
	{
		if (!empty($forumusers))
			$forumusers .= ",\n";
		...
		if (!function_exists('get_user_class_color'))
		{
			...
			{
				case UC_SYSOP:
					$username = "<font color=#FF0000>" . $arr["username"] . "</font>";
				break;
				...
				case UC_MODERATOR:
					$username = "<font color=#6B001A>" . $arr["username"] . "</font>";
				break;

				case UC_UPLOADER:
					$username = "<font color=#9A6258>" . $arr["username"] . "</font>";
				break;

				case UC_VIP:
					$username = "<font color=#FF6600>" . $arr["username"] . "</font>";
				break;

				case UC_POWER_USER:
					$username = "<font color=#4F3157>" . $arr["username"] . "</font>";
				break;

				case UC_USER:
					$username = "<font color=black>" . $arr["username"] . "</font>";
				break;
			}
		}
		else
...
function show_forums($forid)
{
	...
	$forums_res = mysql_query("SELECT f.id, f.name, f.description, f.postcount, f.topiccount, f.minclassread, p.added, p.topicid, p.userid, p.id AS pid, u.username, t.subject,t.smile, t.lastpost, r.lastpostread ".
							  "FROM forums AS f ".
	...
	while ($forums_arr = mysql_fetch_assoc($forums_res))
	{
		...
		if (is_valid_id($forums_arr['pid']))
		{
			$lastpost = "<nobr>".$forums_arr["added"]."<br />" .
						"by <a href='$DEFAULTBASEURL/userdetails.php?id=".(int)$forums_arr["userid"]."'><b>".htmlspecialchars($forums_arr['username'])."</b></a><br />" .
						"<img alt='' src=\"pic/$smile\" /> in <a href='".$_SERVER['PHP_SELF']."?action=viewtopic&topicid=".(int)$forums_arr["topicid"]."&amp;page=p$lastpostid#$lastpostid'><b>".htmlspecialchars($forums_arr['subject'])."</b></a></nobr>";
...
function insert_quick_jump_menu($currentforum = 0)
{
	...
	<input type='submit' class=groovybutton2 value='Go!' class='gobutton'>
	</div>
	</form>
	</p>
	<?php
...
function insert_compose_frame($id, $newtopic = true, $quote = false, $attachment = false)
{
	...
	if ($newtopic)
	{
		...
		?><h3><center>New topic in <a href='<?php echo $_SERVER['PHP_SELF']; ?>?action=viewforum&forumid=<?php echo $id; ?>'><?php echo htmlspecialchars($arr["name"]); ?></a> forum</h3></center><?php
	}
	else
	{
	...
	?><form method='post' name='compose' action='<?php echo $_SERVER['PHP_SELF']; ?>' enctype='multipart/form-data'>
	...
	<td class='rowhead' width="10%">Body</td>
		...
		{
			?><textarea name="body" style="width:100%" rows="7"><?php echo $qbody; ?></textarea><?php
		}
		if ($newtopic)
		tr("<b>Post Icons</b>",

"<center><input type='radio' name='smile'  value='icon14.gif' />&nbsp;<img border='0' src='pic/icon14.gif'  />&nbsp;

<input type='radio' name=smile" .  " value='icon13.gif' />&nbsp;<img alt='' border='0' src='pic/icon13.gif'  />&nbsp;

<input type='radio' name=smile" .  " value='cool2.gif' />&nbsp;<img alt='' border='0' src='pic/icon12.gif'  />&nbsp;

<input type='radio' name=smile" .  " value='icon11.gif' />&nbsp;<img alt='' border='0' src='pic/icon11.gif'  />&nbsp;

<input type='radio' name=smile" .  " value='icon10.gif' />&nbsp;<img alt='' border='0' src='pic/icon10.gif'  />&nbsp;

<input type='radio' name=smile" .  " value='icon9.gif' />&nbsp;<img alt='' border='0' src='pic/icon9.gif'  />&nbsp;

<input type='radio' name=smile" .  " value='icon8.gif' />&nbsp;<img alt='' border='0' src='pic/icon8.gif'  />&nbsp;

<input type='radio' name=smile" .  " value='icon7.gif' />&nbsp;<img alt='' border='0' src='pic/icon7.gif'  />&nbsp;

<input type='radio' name=smile" .  " value='icon6.gif' />&nbsp;<img alt='' border='0' src='pic/icon6.gif'  />&nbsp;

<input type='radio' name=smile" .  " value='icon5.gif' />&nbsp;<img alt='' border='0' src='pic/icon5.gif'  />&nbsp;

<input type='radio' name=smile" .  " value='icon4.gif' />&nbsp;<img alt='' border='0' src='pic/icon4.gif'  />&nbsp;

<input type='radio' name=smile" .  " value='icon3.gif' />&nbsp;<img alt='' border='0' src='pic/icon3.gif'  />&nbsp;

<input type='radio' name=smile" .  " value='icon2.gif' />&nbsp;<img alt='' border='0' src='pic/icon2.gif'  />&nbsp;</center>"
,1);
else
tr("<b>Post Icons</b>",
...
if ($action == 'updatetopic' && $CURUSER['class'] >= UC_MODERATOR)
{
	...
	$topic_res = mysql_query('SELECT t.sticky, t.locked, t.subject, t.forumid, f.minclasswrite, '.
...
else if ($action == "post") //-------- Action: Post
{
	...
	if ($newtopic)
	{
		mysql_query("INSERT INTO topics (userid, forumid, subject,smile) VALUES($userid, $forumid, ".sqlesc($subject).",$smile)") or sqlerr(__FILE__, __LINE__);
...
$text = "[color=red]New topic in[/color] [url=".$BASEURL."/forums.php?action=viewtopic&topicid=".$topicid."]".$subject."[/url]" ;
$date=time();
...
$smile = sqlesc($smile);
		...
		{
			mysql_query("INSERT INTO posts (topicid, userid, added, body,smile) VALUES($topicid, $userid, ".sqlesc(get_date_time()).", ".sqlesc($body).",$smile)") or sqlerr(__FILE__, __LINE__);
	...
	if ($use_attachment_mod && ((isset($_POST['uploadattachment']) ? $_POST['uploadattachment'] : '') == 'yes'))
	{
		...
		if (empty($uploaderror))
		{
			...
			mysql_query("UPDATE topics SET atach = 'yes' WHERE  id=$topicid") or sqlerr();
...
else if ($action == "viewtopic") //-------- Action: View topic
{
	...
	//------ Get topic info
	$res = mysql_query("SELECT ".($use_poll_mod ? 't.pollid, ' : '')."t.locked, t.subject, t.sticky, t.userid AS t_userid, t.forumid, f.name AS forum_name, f.minclassread, f.minclasswrite, f.minclasscreate, (SELECT COUNT(id) FROM posts WHERE topicid = t.id) AS p_count ".
					   "FROM topics AS t ".
					   "LEFT JOIN forums AS f ON f.id = t.forumid ".
					   "WHERE t.id = ".sqlesc($topicid)) or sqlerr(__FILE__, __LINE__);
	...
	stdhead("Forums :: View Topic: $subject"); begin_main_frame();
		...
		if (mysql_num_rows($res) > 0)
		{
			...
			if (($locked && $CURUSER['class'] < UC_MODERATOR) ? true : $voted)
			{
				...
				for ($i=0; $a = $os[$i]; ++$i)
				{
					...
					$c = ($i % 2 ? '' : "poll");
						?><tr><?php
					?><td width='1%' style="padding:3px;"  class='forum'  class='embedded<?php echo $c; ?>'><nobr><?php echo htmlspecialchars($a[1]); ?></nobr></td><?php
					?><td width='99%'  class='forum'  class='embedded<?php echo $c; ?>' align="center"><?php
			...
			else
			{
				...
				?><br /><?php
				?><p align=center><input type=submit class=groovybutton2 value='Vote!'></p><?php
			}
			...
			if ($CURUSER['class'] >= UC_ADMINISTRATOR)
			{
				...
				{
					$res4 = mysql_query("SELECT pa.userid, u.username FROM postpollanswers AS pa LEFT JOIN users AS u ON u.id = pa.userid WHERE pa.pollid = ".sqlesc($pollid)) or sqlerr(__FILE__, __LINE__);
					$voters = '';
					while ($arr4 = mysql_fetch_assoc($res4))
					{
						...
						$voters .= "<a href='$DEFAULTBASEURL/userdetails.php?id=".(int)$arr4['userid']."'><b>".htmlspecialchars($arr4['username'])."</b></a>";
					}
	...
	?><a name='top'></a>
    <h1 align="left"><center><a href="<?php echo $_SERVER['PHP_SELF']; ?>?action=viewforum&forumid=<?php echo $forumid; ?>"><?php echo $forum; ?></a> &gt; <?php echo htmlspecialchars($subject); ?></h1><p></p></center><?php
	...
	while ($arr = mysql_fetch_assoc($res))
	{
		...
		begin_table();
		?><tr><td width='100%' colspan="2"><table class="main"><tr><td style="border:none;" width="100%"><a href='<?php echo $_SERVER['PHP_SELF']; ?>?action=viewtopic&topicid=<?=$topicid;?>&page=p<?=$postid;?>#<?=$postid;?>'>#<?=$postid;?></a> by <?=$by;?> <?=$title;?> <?echo("<img alt='' src=\"pic/$smile\" />\n");?>at <?php echo $added; ?><?php
		?></td><td style="border:none;"><a href="#top"><img align="right" src='<?php echo $pic_base_url.$forum_pics['arrow_up']; ?>' alt='Top' /></a></td></tr></table></td></tr><?php
		...
		<img width='167' src="<?=$avatar;?>" height="161"><br /><fieldset style='text-align:left;border:none;'><nobr>
		 <table width="90%"  align="center"><tr><td><b>Posts:</b>&nbsp;&nbsp;&nbsp;<?=$forumposts;?><br />
		<b>Ratio:</b>&nbsp;&nbsp;&nbsp;<?=$ratio;?><br />
		<b>Uploaded:</b>&nbsp;&nbsp;&nbsp;<?=$uploaded;?><br />
		<b>Downloaded:</b>&nbsp;&nbsp;&nbsp;<?=$downloaded;?></td></table>
			<?echo("<br><center><a href=takesimpaty.php?id=$arr[userid]&vote=good><img src=pic/thumb_up.png border=0></a> (" . htmlspecialchars(trim($arr["good"])). ")&nbsp;&nbsp;&nbsp; <a href=takesimpaty.php?id=$arr[userid]&vote=bad><img src=pic/thumb_down.png border=0></a> (" . htmlspecialchars(trim($arr["bad"])). ")&nbsp; </center></br>\n");
?>
		</nobr>
		</fieldset></td><td class='text' width='100%'><?=$body;?></td></tr><?php
		...
		?><tr>
			...
			<img src='<?php echo $pic_base_url.$forum_pics[($last_access > get_date_time(gmtime()-360) || $posterid == $CURUSER['id'] ? 'on' : 'off').'line_btn']; ?>' border=0>&nbsp;
				...
				<a href="<?php echo $DEFAULTBASEURL; ?>/messages.php?action=send&receiver=<?php echo $posterid; ?>"><img src="<?php  echo $pic_base_url.$forum_pics['pm_btn']; ?>" border="0" alt="PM" <?php echo htmlspecialchars($postername); ?>"></a>&nbsp;
				<a href='<?php echo $DEFAULTBASEURL; ?>/report.php?type=Post&id=<?php echo $postid; ?>&id_2=<?php echo $topicid; ?>&id_3=<?php echo $posterid; ?>'><img src="<?php echo $pic_base_url.$forum_pics['p_report_btn']; ?>" border="0" alt="Report Post"></a>
			</td>
			<td align='right'>
		<?php
	...
	if ($use_poll_mod && (($userid == $t_userid || $CURUSER['class'] >= UC_MODERATOR) && !is_valid_id($pollid)))
	{
        ...
        <tr>
        	<td align="right">
				...
				<input type='submit' class=groovybutton2 value='Add a Poll'>
				</form>
			</td>
        </tr>
        </table>
	...
	if ($CURUSER['class'] >= UC_MODERATOR)
	{
		...
		<tr>
			<td colspan="2" align="center">
				<input type="submit" class=groovybutton value="Update Topic" />
			</td>
		</tr>
	...
	<td class=embedded>
		<form method=get action='<?php echo $_SERVER['PHP_SELF']; ?>'><input type=hidden name=action value=viewunread><input type=submit class=groovybutton2 value='Show new'></form>
	</td>
	<?php
	if ($maypost)
	{
		...
		<td class=embedded style='padding-left: 10px'>
			...
			<input type=hidden name=action value=reply><input type=hidden name=topicid value=<?php echo $topicid; ?>><input type=submit class=groovybutton2 value='Answer'></form>
		</td>
		<?php
	}
	?></tr></table><?php
	...
	{
		?>
		<p></p><table style='border:1px solid #000000;' align="center"><tr>
...
else if ($action == "editpost") //-------- Action: Edit post
{
	...
	<form name=edit method=post action='<?php echo $_SERVER['PHP_SELF']; ?>?action=editpost&postid=<?php echo $postid; ?>'>
	...
	<tr>
		<td align=center colspan=2><input type=submit class=groovybutton value='Update post' class=gobutton></td>
	</tr>
	</table>
...
else if ($action == 'deletepost' && $CURUSER['class'] >= UC_MODERATOR) //-------- Action: Delete post
{
    ...
    $res = mysql_query("SELECT p.topicid".($use_attachment_mod ? ", a.filename" : "").", (SELECT COUNT(id) FROM posts WHERE topicid=p.topicid) AS posts_count, ".
    ...
    update_topic_last_post($topicid);
 mysql_query("UPDATE topics SET atach = 'no' WHERE $topicid") or sqlerr();
...
else if ($use_poll_mod && ($action == 'deletepoll' && $CURUSER['class'] >= UC_MODERATOR))
{
	...
	mysql_query("UPDATE topics SET polls = 'no' WHERE pollid = ".sqlesc($polls));
...
else if ($use_poll_mod && $action == 'makepoll')
{
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST" && !$topicid)
	{
		...
		else
		{
			...
			mysql_query("UPDATE topics SET polls = 'yes' WHERE  id=$topicid") or sqlerr();
		}
	...
	<tr><td colspan=2 align=center><input type=submit class=groovybutton value='<?php echo ($pollid ? 'Edit poll' : 'Create poll'); ?>' style='height: 20pt'></td></tr>
	</table>
...
else if ($use_attachment_mod && $action == "attachment")
{
	...
	if ((isset($_GET['subaction']) ? $_GET['subaction'] : '') == 'delete')
	{
		...
		mysql_query("UPDATE topics SET atach = 'no' WHERE $id") or sqlerr();
mysql_query("UPDATE topics SET atach = 'no' WHERE $id") or sqlerr();
...
else if ($action == "viewforum") //-------- Action: View forum
{
	...
	"SELECT t.id, t.userid, t.views, t.locked,t.smile, t.sticky,t.atach,t.polls".($use_poll_mod ? ', t.pollid' : '').", t.subject, u1.username, r.lastpostread, p.id AS p_id, p.userid AS p_userid, p.added AS p_added, (SELECT COUNT(id) FROM posts WHERE topicid=t.id) AS p_count, u2.username AS u2_username ".
	"FROM topics AS t ".
	...
	"WHERE t.forumid = ".sqlesc($forumid)." ORDER BY t.sticky,t.polls,t.smile,t.atach, t.lastpost ASC LIMIT $offset, $perpage") or sqlerr(__FILE__, __LINE__);

	stdhead("Forum"); begin_main_frame();

	?><h1 align="center"><?php echo htmlspecialchars($arr["forum_name"]); ?></h1><p></p><?php
	...
	if (mysql_num_rows($topics_res) > 0)
	{
		...
		while ($topic_arr = mysql_fetch_assoc($topics_res))
		{
			...
			$polls = ($topic_arr['polls'] == "yes");
			$smile = ($topic_arr['smile']);
			if ($topic_arr["smile"] == "")
            $smile ="icon1.gif";
            $atach = ($topic_arr['atach'] == "yes");
				...
				<td align=left width="100%">
					<table border=0 cellspacing=0 cellpadding=0>
						<tr>
							...
							<td class=embedded align=left><a href='<?php echo $_SERVER['PHP_SELF']; ?>?action=viewtopic&topicid=<?php echo $topicid; ?>'><?php echo htmlspecialchars($topic_arr['subject']); ?></a>&nbsp;&nbsp;<?php echo ($sticky ? "<img src=pic/sticky.gif>  " : ""); ?><?php echo ($polls ? "<i></i><img alt='Poll' src=pic/poll_posticon.gif>  " : ""); ?><?php echo ($atach ? "<i></i><img alt='attachments' src=pic/paperclip.gif>  " : ""); ?><?php echo $topicpages; ?></td>
						</tr>
					</table>
				</td>
				<?echo("<td align=center><img alt='' src=\"pic/$smile\" /></td>\n");?>
	...
	<td class=embedded><form method=get action='<?php echo $_SERVER['PHP_SELF']; ?>'><input type=hidden name=action value=viewunread><input type=submit class=groovybutton value='View unread' ></form></td>
	<?php
	if ($maypost)
	{
		?>
		<td class=embedded><form method=get action='<?php echo $_SERVER['PHP_SELF']; ?>'><input type=hidden name=action value=newtopic><input type=hidden name=forumid value=<?php echo $forumid; ?>><input type=submit class=groovybutton value='New topic' class=gobutton style='margin-left: 10px'></form></td>
		<?php
	}
...
else if ($action == "getdaily")
{
	$res = mysql_query('SELECT COUNT(p.id) AS post_count '.
					   ...
					   'LEFT JOIN topics AS t ON t.id = p.topicid '.
					   'LEFT JOIN forums AS f ON f.id = t.forumid '.
					   'WHERE ADDDATE(p.added, INTERVAL 1 DAY) > '.sqlesc(get_date_time()).' AND f.minclassread <= '.$CURUSER['class']) or sqlerr(__FILE__, __LINE__);
	...
	$res = mysql_query('SELECT p.id AS pid, p.topicid, p.userid AS userpost, p.added, t.id AS tid, t.subject, t.forumid, t.lastpost, t.views, f.name, f.minclassread, f.topiccount, u.username '.
					   ...
					   'WHERE ADDDATE(p.added, INTERVAL 1 DAY) > '.sqlesc(get_date_time()).' AND f.minclassread <= '.$CURUSER['class'].' '.
	...
	while ($getdaily = mysql_fetch_assoc($res))
	{
		...
		?><tr><?php
			...
			?><td align="center"><?php
				...
				{
					?><a href="<?php echo $DEFAULTBASEURL; ?>/userdetails.php?id=<?php echo $posterid; ?>"><?php echo htmlspecialchars($getdaily['username']); ?></a><?php
				}
				else
				{
		...
		else
		{
			...
			for ($i = 0; $i < $num; ++$i)
			{
				...
				echo "<tr>".
						...
						"<td align=left><nobr>".(empty($post['username']) ? 'unknown['.$post['userid'].']' : "<b><a href='$DEFAULTBASEURL/userdetails.php?id=".$post['userid']."'>".$post['username']."</a></b>")."<br />at ".$post['added']."</nobr></td>".
					 "</tr>";
			}
			end_table();
...
else //-------- Default action: View forums
{
	...
	while ($ovf_arr = mysql_fetch_assoc($ovf_res))
	{
		...
		?><tr>
			<td align='left' class='forum' width="100%">
				<a href='<?php echo $_SERVER['PHP_SELF']; ?>?action=forumview&forid=<?php echo $ovfid; ?>'><b><font color='black'><?php echo htmlspecialchars($ovfname); ?></font></b></a>
			</td>
			<td align='right' class='forum'><font color='white'><b><font color="#000000">Topics</font></b></td>
			<td align='right' class='forum'><font color='white'><b><font color="#000000">Posts</font></b></font></td>
			<td align='left' class='forum'><font color='white'><b><font color="#000000">Last post</font></b></font></td>
		</tr><?php
