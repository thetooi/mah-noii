...
echo("<tr><td align=center colspan=1><form method=\"get\" action=\"moforums.php#add\"></form><form method=\"get\" action=\"moforums.php#add\"><input type=\"submit\" value=\"SubForum Manager\" class=\"btn\" /></form></td></tr>\n");
end_frame(); ?>
...
<tr align="center">
  ...
  <tr>
    ...
    <?
             //$maxclass = get_user_class();
          //for ($i = 0; $i <= $maxclass; ++$i)
            //echo("<option value=$i" . ($row["minclassread"] == $i ? " selected" : "") . ">$prefix" . get_user_class_name($i) . "\n");
...
?>
  ...
  <tr>
    ...
    <?
              $maxclass = get_user_class();
          for ($i = 0; $i <= $maxclass; ++$i)
            echo("<option value=$i" . ($row["minclasswrite"] == $i ? " selected" : "") . ">$prefix" . get_user_class_name($i) . "\n");
?>
        </select></td>
  </tr>
  <tr>
    ...
    <?
            $maxclass = get_user_class();
          for ($i = 0; $i <= $maxclass; ++$i)
            echo("<option value=$i" . ($row["minclasscreate"] == $i ? " selected" : "") . ">$prefix" . get_user_class_name($i) . "\n");
?>
        </select></td>
  </tr>
    <tr>
...
echo("<tr><td align=center colspan=1><form method=\"get\" action=\"forummanage.php#add\"><input type=\"submit\" value=\"Return\" class=\"btn\" /></form></td></tr>\n");
end_frame();
end_main_frame();
}
echo("</table>");
