...
if ($mailbox != PM_INBOX && $mailbox != PM_SENTBOX){
    $res = mysqli_query($GLOBALS["___mysqli_ston"], 'SELECT name FROM pmboxes WHERE userid=' . sqlesc($CURUSER['id']) . ' AND boxnumber=' . sqlesc($mailbox) . ' LIMIT 1') or sqlerr(__FILE__,__LINE__);
...
if ($mailbox == PM_INBOX)
    ...
    $pmcount = mysqli_fetch_row(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT COUNT(*) FROM messages WHERE receiver=".$CURUSER['id']." AND location >= '1' || sender=".$CURUSER['id']." AND saved = 'yes' ")) or sqlerr(__FILE__,__LINE__);
...
if ($mailbox != PM_SENTBOX){
    $res = mysqli_query($GLOBALS["___mysqli_ston"], 'SELECT * FROM messages WHERE receiver=' . sqlesc($CURUSER['id']) . ' AND location=' . sqlesc($mailbox) . ' ORDER BY id DESC') or sqlerr(__FILE__,__LINE__);
    }else{
    $res = mysqli_query($GLOBALS["___mysqli_ston"], 'SELECT * FROM messages WHERE sender=' . sqlesc($CURUSER['id']) . ' AND saved=\'yes\' ORDER BY id DESC') or sqlerr(__FILE__,__LINE__);
}
if (mysqli_num_rows($res) == 0){
...
if ($row['sender'] != 0 && $row['sender'] != $CURUSER['id']){
    $res2 = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT username FROM users WHERE id=" . sqlesc($row['sender']));
...
if ($friend){
    ...
    $res2 = mysqli_query($GLOBALS["___mysqli_ston"], "select username FROM users WHERE id=" . sqlesc($row['receiver']));
    $username = mysqli_fetch_array($res2);
...
if ($row['unread'] == 'yes'){
    $HTMLOUT .= "<tr>\n<td align='center'><img src=\"pic/mailbox/unreadpm.gif\" title='{$lang['messages_unread']}' alt=\"{$lang['messages_unread_title']}\" /></td>\n";
    }else{
    $HTMLOUT .= "<tr>\n<td align='center'><img src='pic/mailbox/readpm.gif' title='{$lang['messages_read']}e' alt='{$lang['messages_read_title']}' /></td>\n";
}
    ...
    $res = mysqli_query($GLOBALS["___mysqli_ston"], 'select * FROM pmboxes WHERE userid=' . sqlesc($CURUSER['id']) . ' ORDER BY boxnumber') or sqlerr(__FILE__,__LINE__);
    while ($row = mysqli_fetch_assoc($res)){
    ...
    <div align='left'><img src='pic/mailbox/unreadpm.gif' title='{$lang['messages_unread_msg']}' alt='{$lang['messages_unread_title']}' /> {$lang['messages_unread_msg']}<br />
    <img src='pic/mailbox/readpm.gif' title='{$lang['messages_read_msg']}' alt='{$lang['messages_read_title']}' /> {$lang['messages_read_msg']}</div>
    <div align='right'>
...
if (!$pm_id){
    ...
    $res = mysqli_query($GLOBALS["___mysqli_ston"], "select * FROM messages WHERE id=" . sqlesc($pm_id)." AND (receiver=". sqlesc($CURUSER['id']) . " OR (sender=" . sqlesc($CURUSER['id']) . " AND saved='yes')) LIMIT 1") or sqlerr(__FILE__,__LINE__);
if (!$res){
    stderr("{$lang['messages_error']}","{$lang['messages_access']}");
}
    $message = mysqli_fetch_assoc($res) or header("Location: messages.php");
if ($message['sender'] == $CURUSER['id']){
    $res2 = mysqli_query($GLOBALS["___mysqli_ston"], "select username FROM users WHERE id=" . sqlesc($message['receiver'])) or sqlerr(__FILE__,__LINE__);
...
if ($message['sender'] == 0){
    ...
    $res2 = mysqli_query($GLOBALS["___mysqli_ston"], "select username FROM users WHERE id=" . sqlesc($message['sender'])) or sqlerr(__FILE__,__LINE__);
    $sender = mysqli_fetch_array($res2);
...
if ($message['unread'] === 'yes'){
    @mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE messages SET unread='no' WHERE id=" . sqlesc($pm_id) . " AND receiver=" . sqlesc($CURUSER['id']) . " LIMIT 1");
}
    $HTMLOUT = '';
...
if ($_POST['move']){
if ($pm_id){
    @mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE messages SET location=" . sqlesc($pm_box) . " WHERE id=" . sqlesc($pm_id) . " AND receiver=" . $CURUSER['id'] . " LIMIT 1");
    }else{
    @mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE messages SET location=" . sqlesc($pm_box) . " WHERE id IN (" . implode(", ", array_map("sqlesc",$pm_messages)) . ') AND receiver=' .
    $CURUSER['id']);
}
...
if ($pm_id){
    $res = mysqli_query($GLOBALS["___mysqli_ston"], "select * FROM messages WHERE id=" . sqlesc($pm_id)) or sqlerr(__FILE__,__LINE__);
    $message = mysqli_fetch_assoc($res);
if ($message['receiver'] == $CURUSER['id'] && $message['saved'] == 'no'){
    mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM messages WHERE id=" . sqlesc($pm_id)) or sqlerr(__FILE__,__LINE__);
    }elseif ($message['sender'] == $CURUSER['id'] && $message['location'] == PM_DELETED){
    mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM messages WHERE id=" . sqlesc($pm_id)) or sqlerr(__FILE__,__LINE__);
    }elseif ($message['receiver'] == $CURUSER['id'] && $message['saved'] == 'yes'){
    mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE messages SET location=0 WHERE id=" . sqlesc($pm_id)) or sqlerr(__FILE__,__LINE__);
    }elseif ($message['sender'] == $CURUSER['id'] && $message['location'] != PM_DELETED){
    mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE messages SET saved='no' WHERE id=" . sqlesc($pm_id)) or sqlerr(__FILE__,__LINE__);
}
    }else{
    foreach ($pm_messages as $id){
    $res = mysqli_query($GLOBALS["___mysqli_ston"], "select * FROM messages WHERE id=" . sqlesc((int) $id));
    $message = mysqli_fetch_assoc($res);
if ($message['receiver'] == $CURUSER['id'] && $message['saved'] == 'no'){
    mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM messages WHERE id=" . sqlesc((int) $id)) or sqlerr(__FILE__,__LINE__);
    }elseif ($message['sender'] == $CURUSER['id'] && $message['location'] == PM_DELETED){
    mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM messages WHERE id=" . sqlesc((int) $id)) or sqlerr(__FILE__,__LINE__);
    }elseif ($message['receiver'] == $CURUSER['id'] && $message['saved'] == 'yes'){
    mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE messages SET location=0 WHERE id=" . sqlesc((int) $id)) or sqlerr(__FILE__,__LINE__);
    }elseif ($message['sender'] == $CURUSER['id'] && $message['location'] != PM_DELETED){
    mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE messages SET saved='no' WHERE id=" . sqlesc((int) $id)) or sqlerr(__FILE__,__LINE__);
    }
  }
}
if (@mysqli_affected_rows($GLOBALS["___mysqli_ston"]) == 0){
...
if ($_SERVER['REQUEST_METHOD'] == 'GET'){
    $pm_id = (int) $_GET['id'];
    $res = mysqli_query($GLOBALS["___mysqli_ston"], "select * FROM messages WHERE id=" . sqlesc($pm_id) . " AND (receiver=" . sqlesc($CURUSER['id']) . " OR sender=" . sqlesc($CURUSER['id']) .") LIMIT 1") or sqlerr(__FILE__,__LINE__);
if (!$res){
...
if (mysqli_num_rows($res) == 0){
    ...
    $res = mysqli_query($GLOBALS["___mysqli_ston"], "select username FROM users WHERE id=" . sqlesc($orig) . " OR id=" . sqlesc($from)) or sqlerr(__FILE__,__LINE__);
...
if (mysqli_num_rows($res) == 0){
    ...
    $res = mysqli_query($GLOBALS["___mysqli_ston"], "select id FROM users WHERE LOWER(username)=LOWER(" . sqlesc($username) . ") LIMIT 1");
if (!$res){
...
if ($message['sender'] == 0){
    ...
    $res = mysqli_query($GLOBALS["___mysqli_ston"], "select * FROM users WHERE id=" . sqlesc($message['sender'])) or sqlerr(__FILE__,__LINE__);
...
if ($CURUSER['class'] < UC_MODERATOR){if ($from["acceptpms"] == "yes"){
    $res2 = mysqli_query($GLOBALS["___mysqli_ston"], "select * FROM blocks WHERE userid=$to AND blockid=" . $CURUSER["id"]) or sqlerr(__FILE__, __LINE__);
if (mysqli_num_rows($res2) == 1)
    stderr("{$lang['messages_refused']}", "{$lang['messages_blocked']}");
    }elseif ($from["acceptpms"] == "friends"){
    $res2 = mysqli_query($GLOBALS["___mysqli_ston"], "select * FROM friends WHERE userid=$to AND friendid=" . $CURUSER["id"]) or sqlerr(__FILE__, __LINE__);
if (mysqli_num_rows($res2) != 1)
    ...
    @mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO messages (poster, sender, receiver, added, subject, msg, location, saved) VALUES({$CURUSER["id"]}, {$CURUSER["id"]}, $to, " . time() . ", " . sqlesc($subject) . "," .
...
if ($action == "editmailboxes"){
    $res = mysqli_query($GLOBALS["___mysqli_ston"], "select * FROM pmboxes WHERE userid=" . sqlesc($CURUSER['id'])) or sqlerr(__FILE__,__LINE__);
    $HTMLOUT = '';
...
if ($action2 == "add"){
    ...
    $name2 = $_GET['new2'];
    $name3 = $_GET['new3'];
    $res = mysqli_query($GLOBALS["___mysqli_ston"], "select MAX(boxnumber) FROM pmboxes WHERE userid=" . sqlesc($CURUSER['id']));
...
if (strlen($name1) > 0){
    ++$box;
    @mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO pmboxes (userid, name, boxnumber) VALUES (" . sqlesc($CURUSER['id']) . ", " . sqlesc($name1) . ", $box)") or sqlerr(__FILE__,__LINE__);
}
if (strlen($name2) > 0){
    ++$box;
    @mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO pmboxes (userid, name, boxnumber) VALUES (" . sqlesc($CURUSER['id']) . ", " . sqlesc($name2) . ", $box)") or sqlerr(__FILE__,__LINE__);
}
if (strlen($name3) > 0){
    ++$box;
    @mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO pmboxes (userid, name, boxnumber) VALUES (" . sqlesc($CURUSER['id']) . ", " . sqlesc($name3) . ", $box)") or sqlerr(__FILE__,__LINE__);
}
    header("Location: messages.php?action=editmailboxes");
    exit();
}
if ($action2 == "edit");{
    $res = mysqli_query($GLOBALS["___mysqli_ston"], "select * FROM pmboxes WHERE userid=" . sqlesc($CURUSER['id']));
if (!$res){
...
if (strlen($_GET['edit' . $row['id']]) > 0){
    @mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE pmboxes SET name=" . sqlesc($_GET['edit' . $row['id']]) . " WHERE id=" . sqlesc($row['id']) . " LIMIT 1");
    }else{
    @mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM pmboxes WHERE id=" . sqlesc($row['id']) . " LIMIT 1");
    @mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE messages SET location=0 WHERE saved='yes' AND location=" . sqlesc($row['boxnumber']) . " AND receiver=" . sqlesc($CURUSER['id']));
    @mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE messages SET saved='no' WHERE saved='yes' AND sender=" . sqlesc($CURUSER['id']));
    @mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM messages WHERE saved='no' AND location=" . sqlesc($row['boxnumber']) . " AND receiver=" . sqlesc($CURUSER['id']));
    @mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM messages WHERE location=0 AND saved='yes' AND sender=" . sqlesc($CURUSER['id']));
    }
  }
 }
}
...
if (mysqli_num_rows($res) == 0){
    ...
    $message = mysqli_fetch_assoc($res);
if ($message['receiver'] == $CURUSER['id'] && $message['saved'] == 'no'){
    $res2 = mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM messages WHERE id=" . sqlesc($pm_id)) or sqlerr(__FILE__,__LINE__);
    }elseif ($message['sender'] == $CURUSER['id'] && $message['location'] == PM_DELETED){
    $res2 = mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM messages WHERE id=" . sqlesc($pm_id)) or sqlerr(__FILE__,__LINE__);
    }elseif ($message['receiver'] == $CURUSER['id'] && $message['saved'] == 'yes'){
    $res2 = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE messages SET location=0 WHERE id=" . sqlesc($pm_id)) or sqlerr(__FILE__,__LINE__);
    }elseif ($message['sender'] == $CURUSER['id'] && $message['location'] != PM_DELETED){
    $res2 = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE messages SET saved='no' WHERE id=" . sqlesc($pm_id)) or sqlerr(__FILE__,__LINE__);
}
if (!$res2){
...
global $CURUSER, $lang;
    $htmlout = '';
    $res = mysqli_query($GLOBALS["___mysqli_ston"], "select * FROM pmboxes WHERE userid=" . sqlesc($CURUSER['id']) ." ORDER BY boxnumber");
