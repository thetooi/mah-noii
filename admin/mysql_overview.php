...
if (preg_match('@^(CHECK|ANALYZE|REPAIR|OPTIMIZE)[[:space:]]TABLE[[:space:]]'.$Table.'$@i', $sql)){
    @mysqli_query($GLOBALS["___mysqli_ston"], $sql) or sqlerr(__FILE__,__LINE__);
    header("Location: {$FMED['baseurl']}/admin.php?action=mysql_overview&Do=F");
    exit;
    }
}
...
if ($unit != $GLOBALS['byteUnits'][0]){
    ...
    <!-- End table headers -->";
    $count = 0;
    $res = @mysqli_query($GLOBALS["___mysqli_ston"], "SHOW TABLE STATUS FROM {$FMED['mysql_db']}") or stderr(__FILE__,__LINE__);
