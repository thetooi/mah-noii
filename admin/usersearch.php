...
//    </temporary>   /////////////////////////////////////////////////////
  ...
  $res = mysqli_query($GLOBALS["___mysqli_ston"], $queryc) or sqlerr();
  ...
  $res = mysqli_query($GLOBALS["___mysqli_ston"], $query) or sqlerr();
  ...
  else
  {
    ...
    while ($user = mysqli_fetch_array($res))
    {
      ...
      {
            $nip = ip2long($user['ip']);
        $auxres = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT COUNT(*) FROM bans WHERE $nip >= first AND $nip <= last") or sqlerr(__FILE__, __LINE__);
      ...
      $auxres = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT SUM(uploaded) AS pul, SUM(downloaded) AS pdl FROM peers WHERE userid = ".$user['id']) or sqlerr(__FILE__, __LINE__);
...
//    $auxres = mysql_query("SELECT COUNT(id) FROM posts WHERE userid = ".$user['id']) or sqlerr(__FILE__, __LINE__);
      ...
      $auxres = mysqli_query($GLOBALS["___mysqli_ston"],
      ...
      $auxres = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT COUNT(id) FROM comments WHERE user = ".$user['id']) or sqlerr(__FILE__, __LINE__);
      ...
      <font color='#" . get_user_class_color($user['class']) . "'> " . htmlspecialchars($user['username']) . "</font></a></b>" .
      ($user["donor"] == "yes" ? "<img src='pic/userimages/star.gif' alt=\"Donor\" />" : "") .
      ($user["warned"] == "yes" ? "<img src=\"pic/userimages/warned.gif\" alt=\"Warned\" />" : "") . "</td>
