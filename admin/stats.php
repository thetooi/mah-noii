...
require "include/html_functions.php";
    ...
    $res = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT COUNT(*) FROM torrents") or sqlerr(__FILE__, __LINE__);
    $n = mysqli_fetch_row($res);
    $n_tor = $n[0];
    $res = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT COUNT(*) FROM peers") or sqlerr(__FILE__, __LINE__);
    $n = mysqli_fetch_row($res);
    $n_peers = $n[0];
...
if ($catorder == "lastul")
    ...
    $orderby = "c.name";
    $res = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT c.name, MAX(t.added) AS last, COUNT(DISTINCT t.id) AS n_t, COUNT(p.id) AS n_p
