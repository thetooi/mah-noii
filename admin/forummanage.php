...
global $lang;
    ...
    $HTMLOUT .= "<table width='98%' border='0' align='center' cellpadding='2' cellspacing='0'>";
    $HTMLOUT .= "<tr>
    <td class='colhead' align='left'>{$lang['header_name']}</td>
    ...
    <td class='colhead'>{$lang['header_modify']}</td></tr>";
    $result = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT  * FROM forums ORDER BY sort ASC");
if ( mysqli_num_rows($result) > 0) {
    ...
    $HTMLOUT .= "<td>{$row["topiccount"]}</td>
    <td>{$row["postcount"]}</td>
    <td>{$lang['text_minimal']} " . get_user_class_name($row["minclassread"]) . "</td>
    <td>{$lang['text_minimal']} " . get_user_class_name($row["minclasswrite"]) . "</td>
    <td>{$lang['text_minimal']} " . get_user_class_name($row["minclasscreate"]) . "</td>
    <td align='center' style='white-space: nowrap;'><b><a href='admin.php?action=forummanage&amp;mode=edit&amp;id={$row["id"]}'>{$lang['text_edit']}</a>&nbsp;|&nbsp;<a href='admin.php?action=forummanage&amp;mode=delete&amp;id={$row["id"]}'><font color='red'>{$lang['text_delete']}</font></a></b></td></tr>";
}
    }else
...
$HTMLOUT = '';
    ...
    <tr>
        ...
        <td><select name='sort'>";
    $res = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT sort FROM forums");
...
global $lang;
    ...
    $result = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM forums where id = '$id'");
if (mysqli_num_rows($result) > 0) {
    while($row = mysqli_fetch_assoc($result)){
...
global $lang;
if (!$_POST['name'] && !$_POST['desc']) { header("Location: admin.php?action=forummanage");
   die();
}
    @mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO forums
...
if (!$_POST['name'] && !$_POST['desc'] && !$_POST['id']) { header("Location: admin.php?action=forummanage"); die();}
    @mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE forums SET sort = " .
...
global $lang;
    ...
    $res = @mysqli_query($GLOBALS["___mysqli_ston"], "SELECT id FROM topics WHERE forumid=$id");
...
if (mysqli_num_rows($res) == 0)
    @mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM forums WHERE id=$id");
    (mysqli_affected_rows($GLOBALS["___mysqli_ston"]) > 0) ?
...
if (mysqli_num_rows($res) == 0)
    ...
    $tid[] = $row['id'];
    @mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE topics SET forumid=$forumid WHERE id IN (".join(',' , $tid).")");
