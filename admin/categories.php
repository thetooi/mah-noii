...
if( !is_valid_id($params['new_cat_id']) OR ($params['id'] == $params['new_cat_id']) ){
    ...
    $q = @mysqli_query($GLOBALS["___mysqli_ston"],  "SELECT id FROM categories WHERE id IN($old_cat_id, $new_cat_id)" );
if( 2 != mysqli_num_rows($q) ){
    stderr( 'MOD ERROR', 'That category does not exist or has been deleted' );
}
    @mysqli_query($GLOBALS["___mysqli_ston"],  "UPDATE torrents SET category = $new_cat_id WHERE category = $old_cat_id" );
if( -1 != mysqli_affected_rows($GLOBALS["___mysqli_ston"]) ){
    header( "Location: {$FMED['baseurl']}/admin.php?action=categories" );
    }else{
...
if ( !preg_match( "/^cat_[A-Za-z0-9_]+\.(?:gif|jpg|jpeg|png)$/i", $params['new_cat_image'] ) ){
    ...
    @mysqli_query($GLOBALS["___mysqli_ston"],  "INSERT INTO categories (name, cat_desc, image) VALUES($cat_name, $cat_desc, $cat_image)" );
if( -1 == mysqli_affected_rows($GLOBALS["___mysqli_ston"]) ){
    stderr( 'MOD ERROR', 'That category does not exist or has been deleted' );
    }else{
    header( "Location: {$FMED['baseurl']}/admin.php?action=categories" );
...
if( !is_valid_id($params['new_cat_id']) OR ($r['id'] == $params['new_cat_id']) ){
    ...
    $new_cat_id = intval($params['new_cat_id']);
    $q = @mysqli_query($GLOBALS["___mysqli_ston"],  "SELECT COUNT(*) FROM categories WHERE id = $new_cat_id" );
...
if( !$count[0] ){
    ...
    @mysqli_query($GLOBALS["___mysqli_ston"],  "DELETE FROM categories WHERE id = $old_cat_id" );
...
if( false == mysqli_num_rows($q) ){
    ...
    $r = mysqli_fetch_assoc($q);
    $q = @mysqli_query($GLOBALS["___mysqli_ston"],  "SELECT COUNT(*) FROM torrents WHERE category = ".intval($r['id']) );
    $count = mysqli_fetch_array($q,  MYSQLI_NUM);
    $check = '';
if($count[0]){
    $select = "<select name='new_cat_id'>\n<option value='0'>Select Category</option>\n";
...
if ( !preg_match( "/^cat_[A-Za-z0-9_]+\.(?:gif|jpg|jpeg|png)$/i", $params['cat_image'] ) ){
    ...
    @mysqli_query($GLOBALS["___mysqli_ston"],  "UPDATE categories SET name = $cat_name, cat_desc = $cat_desc, image = $cat_image WHERE id = $cat_id" );
if( -1 == mysqli_affected_rows($GLOBALS["___mysqli_ston"]) ){
    stderr( 'MOD ERROR', 'That category does not exist or has been deleted' );
    }else{
    header( "Location: {$FMED['baseurl']}/admin.php?action=categories" );
...
if( false == mysqli_num_rows($q) ){
    ...
    $r = mysqli_fetch_assoc($q);
    $dh = opendir( $FMED['pic_base_url'].'caticons' );
    $files = array();
...
if( is_array($files) AND count($files) ){
    ...
    $query = @mysqli_query($GLOBALS["___mysqli_ston"],  "SELECT * FROM categories" );
if( false == mysqli_num_rows($query) ){
    $htmlout .= "<tr><td colspan='7'><h1>No categories defined.</h1></td></tr>";
    }else{
    while($row = mysqli_fetch_assoc($query)){
    $cat_image = file_exists($FMED['pic_base_url'].'caticons/'.$row['image']) ? "<img border='0' src='{$FMED['pic_base_url']}caticons/{$row['image']}' alt='{$row['id']}' />" : "No Image";
    $htmlout .= "<tr>
    ...
    <img src='{$FMED['pic_base_url']}aff_tick.gif' alt='Edit Category' title='Edit' width='12' height='12' border='0' /></a></td>
    <td align='center' width='18'><a href='admin.php?action=categories&amp;mode=del_cat&amp;id={$row['id']}'>
    <img src='{$FMED['pic_base_url']}aff_cross.gif' alt='Delete Category' title='Delete' width='12' height='12' border='0' /></a></td>
    <td align='center' width='18'><a href='admin.php?action=categories&amp;mode=move_cat&amp;id={$row['id']}'>
    <img src='{$FMED['pic_base_url']}plus.gif' alt='Move Category' title='Move' width='12' height='12' border='0' /></a></td>
    </tr>";
    }
}
    $htmlout .= '</table>';
