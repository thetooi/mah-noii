...
loggedinorreturn();
   ...
   $pm_std_reply[1] = sprintf( $lang['sendmessage_std_reply1'], $FMED['baseurl'] );
   $pm_std_reply[2] = "{$lang['sendmessage_std_reply2']}";
   $pm_template[1] = array( $lang['sendmessage_template1_sub'], sprintf($lang['sendmessage_template1_body'], $FMED['site_name']) );
   $pm_template[2] = array( $lang['sendmessage_template2_sub'], sprintf($lang['sendmessage_template2_body'], $FMED['baseurl']) );
   $mm_template[1] = array( $lang['sendmessage_mm_template1_sub'], sprintf($lang['sendmessage_mm_template1_body'], $FMED['site_name']) );
...
if ($auto){
    ...
    $HTMLOUT .= "<h1>{$mass_msg_pm_to}</h1><form method='post' action='takemessage.php'>";
...
if (($auto || $std ) && $CURUSER['class'] < UC_MODERATOR)
    ...
    $res = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM users WHERE id=$receiver") or die(mysqli_error($GLOBALS["___mysqli_ston"]));
...
if ($replyto){
    $res = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM messages WHERE id=$replyto") or sqlerr();
    $msga = mysqli_fetch_assoc($res);
if ($msga['receiver'] != $CURUSER['id'])
    die;
    $res = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT username FROM users WHERE id={$msga['sender']}") or sqlerr();
...
if ($replyto){
    ...
    <input type='checkbox' name='save' value='yes' ".($CURUSER['savepms'] == 'yes' ? "checked='checked'":'')." />{$lang['sendmessage_save_sent']}</td></tr>
    <tr>
    <td".($replyto ? " colspan='2'":'')." align='center'><input type='submit' value='{$lang['sendmessage_send_it']}' class='btn' /></td></tr>
    </table>
