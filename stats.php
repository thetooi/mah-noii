...
else
{
	...
	{
		echo("<tr><td><a href=userdetails.php?id=".$uper['id']."><b>".$uper['name']."</b></a></td>\n");
		echo("<td " . ($uper['last']?(">".$uper['last']." (".get_elapsed_time(sql_timestamp_to_unix_timestamp($uper['last']))." ago)"):"align=center>---") . "</td>\n");
		echo("<td align=right>" . $uper['n_t'] . "</td>\n");
		echo("<td align=right>" . ($n_tor > 0?number_format(100 * $uper['n_t']/$n_tor,1)."%":"---") . "</td>\n");
		echo("<td align=right>" . $uper['n_p']."</td>\n");
		echo("<td align=right>" . ($n_peers > 0?number_format(100 * $uper['n_p']/$n_peers,1)."%":"---") . "</td></tr>\n");
	}
	end_table();
	end_frame();
}
...
else
{
  ...
  $res = mysql_query("SELECT c.name, MAX(t.added) AS last, COUNT(DISTINCT t.id) AS n_t, COUNT(p.id) AS n_p
	...
	{
		echo("<tr><td class=rowhead>" . $cat['name'] . "</b></a></td>");
		echo("<td " . ($cat['last']?(">".$cat['last']." (".get_elapsed_time(sql_timestamp_to_unix_timestamp($cat['last']))." ago)"):"align = center>---") ."</td>");
		echo("<td align=right>" . $cat['n_t'] . "</td>");
		echo("<td align=right>" . number_format(100 * $cat['n_t']/$n_tor,1) . "%</td>");
		echo("<td align=right>" . $cat['n_p'] . "</td>");
		echo("<td align=right>" . ($n_peers > 0?number_format(100 * $cat['n_p']/$n_peers,1)."%":"---") . "</td>\n");
	}
	end_table();
	end_frame();
}
