...
$cat = $arr["name"];
$res = mysql_query("SELECT email FROM users WHERE enabled='yes' AND notifs LIKE '%[cat$catid]%' AND notifs LIKE '%[email]%'") or sqlerr();
...
$text = "[color=red]New Torrent:[/color] [url=".$BASEURL."/details.php?id=".$id."]".$torrent."[/url]" ;
$date=time();
