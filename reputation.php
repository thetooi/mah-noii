...
///////////////////////////////////////////////
        $forum = mysqli_query($GLOBALS["___mysqli_ston"],  "SELECT posts.topicid, posts.userid, forums.minclassread,
...
///////////////////////////////////////////////
        $repeat = mysqli_query($GLOBALS["___mysqli_ston"],  "SELECT postid FROM reputation WHERE postid ={$input['pid']}
                        AND whoadded={$CURUSER['id']}" );

        //$repres = mysql_fetch_assoc( $forum ) or sqlerr(__LINE__,__FILE__);


        if( mysqli_num_rows( $repeat) > 0 ) // blOOdy eedjit check!
        {
...
///////////////////////////////////////////////
            $flood = mysqli_query($GLOBALS["___mysqli_ston"],  "SELECT dateadd, userid FROM reputation
...
///////////////////////////////////////////////
        ...
        $r = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT COUNT(*) FROM posts WHERE userid = {$CURUSER['id']}") or sqlerr();
...
///////////////////////////////////////////////
        if( isset( $input['do'] ) && $input['do']  == 'addrep' )
        {
            ...
            @mysqli_query($GLOBALS["___mysqli_ston"],  "UPDATE users set reputation=".intval($res['reputation']). " WHERE id=" .$res['userid'] );
            ...
            header( "Location: {$FMED['baseurl']}/reputation.php?pid={$input['pid']}&done=1" );
        ...
        else
        {
            ...
            {
                // check for fish!
                $query = mysqli_query($GLOBALS["___mysqli_ston"],  "select r.*, leftby.id as leftby_id, leftby.username as leftby_name
                                        from reputation r
                ...
                if( false !== mysqli_num_rows($query) )
                {
                    ...
                    while( $postrep = mysqli_fetch_assoc($query) )
                    {
                        ...
                        {
                            $postrep['reason'] = $postrep['reason']." <span class='desc'>{$lang["rep_let_by"]} <a href=\"{$FMED['baseurl']}/userdetails.php?id={$postrep['leftby_id']}\" target='_blank'>{$postrep['leftby_name']}</a></span>";
                        }
...
///////////////////////////////////////////////
                $rep_info = sprintf("".$lang["info_your_rep_on"]." <a href='{$FMED['baseurl']}/forums.php?action=viewtopic&amp;topicid=%d&amp;page=p%d#%d' target='_blank'>".$lang["info_this_post"]."</a> ".$lang["info_is"]." %s.", $res['topicid'], $input['pid'], $input['pid'], $rep );
