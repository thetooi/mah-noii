...
function insert_tag($name, $description, $syntax, $example, $remarks)
{
	...
	echo("<tr valign=top><td width=25%>Description:</td><td>$description\n");
	echo("<tr valign=top><td>Syntax:</td><td><tt>$syntax</tt>\n");
	echo("<tr valign=top><td>Example:</td><td><tt>$example</tt>\n");
	echo("<tr valign=top><td>Result:</td><td>$result\n");
	if ($remarks != "")
		echo("<tr><td>Remarks:</td><td>$remarks\n");
	echo("</table>\n");
}
...
begin_main_frame();
begin_frame("Tags");
$test = mysql_escape_string( htmlspecialchars($_POST["test"]));
?>
...
if ($test != "")
  echo("<p><hr>" . format_comment($test) . "<hr></p>\n");
