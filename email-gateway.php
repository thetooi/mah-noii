...
if ( !is_valid_id($id) )
    ...
    $res = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT username, class, email FROM users WHERE id=$id");
...
if ($from_email == "") $from_email = "{$FMED['site_email']}";
...
$message = "Message submitted from {$_SERVER['REMOTE_ADDR']} at " . gmdate("Y-m-d H:i:s") . " GMT.\n" .
        ...
        $message . "\n\n" .
        "---------------------------------------------------------------------\n".
        "{$FMED['site_name']}{$lang['email_gateway']}\n";

    $success = mail($to, $subject, $message, "{$lang['email_from']}{$FMED['site_email']}");
if ($success)
    ...
    <tr>
    <td class='rowhead'>{$lang['email_your_name']}</td>
    <td><input type='text' name='from' size='80' /></td>
    </tr>
    <tr>
    <td class='rowhead'>{$lang['email_your_email']}</td>
    <td><input type='text' name='from_email' size='80' /></td>
    </tr>
    <tr>
    <td class='rowhead'>{$lang['email_subject']}</td>
    <td><input type='text' name='subject' size='80' /></td>
    </tr>
    <tr>
    <td class='rowhead'>{$lang['email_message']}</td>
    <td><textarea name='message' cols='80' rows='20'></textarea></td>
    </tr>
    <tr>
    <td colspan='2' align='center'>
    <input type='submit' value='{$lang['email_send']}' class='btn' /></td></tr>
    </table>
    </form>
    <p>
