...
if ($_GET['h'])
{
	...
	echo ("Fields left blank will be ignored;\n
...
<tr>
  <td valign="middle" class=rowhead>Passkey:</td>
...
if (count($_GET) > 0 && !$_GET['h'])
{
  ...
  // passkey
  $pass = explode(' ', trim($_GET['pass']));
  if ($pass[0] !== "")
  {
  	$where_is .= isset($where_is)?" AND (":"(";
    foreach($pass as $passkey)
    {
	  	if (strpos($passkey,'*') === False && strpos($passkey,'?') === False
	    		&& strpos($passkey,'%') === False)
	    {
	      ...
	      $passkey_is .= (isset($passkey_is)?" OR ":"")."u.passkey =".sqlesc($passkey);
      }
      else
      {
	    	$sql_passkey = str_replace(array('?','*'), array('_','%'), $passkey);
	         $passkey_is .= (isset($passkey_is)?" OR ":"")."u.oldpasskey LIKE ".sqlesc($sql_passkey);
		$passkey_is .= (isset($passkey_is)?" OR ":"")."u.passkey LIKE ".sqlesc($sql_passkey);
	    }
    }
		$where_is .= $passkey_is.")";
    $q .= ($q ? "&amp;" : "") . "em=".urlencode(trim($_GET['pass']));
  }
  ...
  $select_is = "u.id, u.username, u.email, u.status, u.added, u.last_access, u.ip,
  	u.class, u.uploaded, u.downloaded, u.donor, u.modcomment, u.enabled, u.warned, u.passkey, u.oldpasskey";
...
//    </temporary>   /////////////////////////////////////////////////////
  ...
  else
  {
  	...
  	echo '<form method="post" action="admin_delusers.php" id="delusers">';
   echo "<table border=1 cellspacing=0 width=70% cellpadding=5>\n";
...
//      		($user["donor"] == "yes" ? "<img src=".$GLOBALS['pic_base_url']."/star.gif alt=\"Donor\">" : "") .
//					($user["warned"] == "yes" ? "<img src=\"".$GLOBALS['pic_base_url']."/warned.gif\" alt=\"Warned\">" : "") . "</td>
          ...
          "|".($n_comments?"<a href=".$GLOBALS['DEFAULTBASEURL']."/userhistory.php?action=viewcomments&id=".$user['id'].">$n_comments</a>":$n_comments).
          "</div></td>
...
<input type="button" value="Check all" onclick="doCheckbox(document.getElementById('delusers').elements['deleteuser[]'])">
...
print("<p>$pagemenu<br>$browsemenu</p></form>");
