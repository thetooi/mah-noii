...
$body  = "[url=userdetails.php?id=$userid][b]This person[/b][/url] has added you to their Friends List. See all Friend Requests [url=friends.php#pending][b]Here[/b][/url]\n ";
sendPersonalMessage(0, $targetid, " Added you to their Friends List ", $body, PM_FOLDERID_SYSTEM, 0);
...
$body  = "[url=userdetails.php?id=$userid][b]This person[/b][/url] has just confirmed your Friendship Request. See your Friends  [url=friends.php][b]Here[/b][/url]\n ";
sendPersonalMessage(0, $targetid, " Confirmed your Friendship Request ", $body, PM_FOLDERID_SYSTEM, 0);
...
echo("<table class=main width=90% border=0 cellspacing=0 cellpadding=0><tr><td class=embedded>");
...
$online = ($friend["last_access"] >= ''. get_date_time($dt). '' ? '&nbsp;<img src='.$pic_base_url.'button_online.gif border=0 alt=Online>' : '<img src='.$pic_base_url.'button_offline.gif border=0 alt=Offline>');
    ...
    $body1 = "<a href=userdetails.php?id=" . $friend['id'] . "><b>" . $friend['name'] . "</b></a>" .
    	get_user_icons($friend) . " ($title) $online<br /><br />last seen on " . $friend['last_access'] .
    	"<br />(" . get_elapsed_time(sql_timestamp_to_unix_timestamp($friend['last_access'])) . " ago)";
		$body2 = "<br /><a href=friends.php?id=$userid&action=confirm&type=friend&targetid=" . $friend['id'] . ">Confirm</a>" .
        "<br /><a href=friends.php?action=add&type=block&targetid=" . $friend['id'] . ">Block</a>" .
			"<br /><br /><a href=messages.php?action=send&receiver=" . $friend['id'] . ">".$pm_pic."</a>";
    $avatar = ($CURUSER["avatars"] == "yes" ? htmlspecialchars($friend["avatar"]) : "");
//		if (!$avatar)
//			$avatar = "".$pic_base_url."default_avatar.gif";
...
else
{
	...
	while ($friend = mysql_fetch_assoc($res))
	{
		if ($i % 6 == 0)
			$friendreqs .= "<tr>";
    	$friendreqs .= "<td style='border: none; padding: 4px; spacing: 0px;'>[<font class=small><a href=friends.php?id=$userid&action=delpending&type=friend&targetid=" .
				$friend['id'] . ">D</a></font>] <a href=userdetails.php?id=" . $friend['id'] . "><b>" . $friend['name'] . "</b></a>" .
