...
echo("<form method=\"post\" action=\"modarticles.php?act=addsect\">");
...
echo("<tr><td class=rowhead style='padding: 3px'>Articles</td><td>");
?>
...
mysql_query("insert into articles (title, text, public, class) values($title, $text, $public, $class)") or sqlerr(__FILE__,__LINE__);
header("Refresh: 0; url=modarticles.php");
}
elseif ($_GET["act"] == "edit"){
$id =htmlspecialchars( $_POST["id"]);
$res = @mysql_fetch_array(@mysql_query("select * from articles where id='$id'"));
stdhead("Edit Articles");
//echo("<td valign=top style=\"padding: 10px;\" colspan=2 align=center>");
begin_main_frame();

echo("<form method=\"post\" action=\"modarticles.php?act=edited\">");
echo("<table border=\"1\" cellspacing=\"0\" cellpadding=\"10\" align=\"center\">\n");
echo("<tr><td>Title:</td><td><input style=\"width: 400px;\" type=\"text\" name=\"title\" value=\"$res[title]\" /></td></tr>\n");
echo("<tr><td class=rowhead style='padding: 3px'>Rules</td><td>");
?>
...
$public = sqlesc($_POST["public"]);
$class = sqlesc($_POST["class"]);
mysql_query("update articles set title=$title, text=$text, public=$public, class=$class where id=$id") or sqlerr(__FILE__,__LINE__);
header("Refresh: 0; url=modarticles.php");
}
else{
...
echo("<tr><td align=center><a href=modarticles.php?act=newsect>Add Section</a></td></tr></table>\n");
while ($arr=mysql_fetch_assoc($res)){
echo("<br><table width=95% border=1 cellspacing=0 cellpadding=10>");
echo("<form method=post action=modarticles.php?act=edit&id=><tr><td class=colhead>:: $arr[title]</td></tr><tr><td><ul>\n");
