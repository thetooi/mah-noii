...
echo("<form method=\"post\" action=\"modlinks.php?act=addsect\">");
...
echo("<tr><td class=rowhead style='padding: 3px'>Links</td><td>");
?>
...
mysql_query("insert into links (title, text, public, class) values($title, $text, $public, $class)") or sqlerr(__FILE__,__LINE__);
header("Refresh: 0; url=modlinks.php");
}
elseif ($_GET["act"] == "edit"){
$id =htmlspecialchars( $_POST["id"]);
$res = @mysql_fetch_array(@mysql_query("select * from links where id='$id'"));
stdhead("Edit links");
//echo("<td valign=top style=\"padding: 10px;\" colspan=2 align=center>");
begin_main_frame();

echo("<form method=\"post\" action=\"modlinks.php?act=edited\">");
echo("<table border=\"1\" cellspacing=\"0\" cellpadding=\"10\" align=\"center\">\n");
...
$public = sqlesc($_POST["public"]);
$class = sqlesc($_POST["class"]);
mysql_query("update links set title=$title, text=$text, public=$public, class=$class where id=$id") or sqlerr(__FILE__,__LINE__);
header("Refresh: 0; url=modlinks.php");
}
else{
...
echo("<tr><td align=center><a href=modlinks.php?act=newsect>Add Section</a></td></tr></table>\n");
while ($arr=mysql_fetch_assoc($res)){
echo("<br><table width=95% border=1 cellspacing=0 cellpadding=10>");
echo("<form method=post action=modlinks.php?act=edit&id=><tr><td class=colhead>:: $arr[title]</td></tr><tr><td><ul>\n");
