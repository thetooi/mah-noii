...
if ($color)
    ...
    $htmlout .= "<tr$highlight><td align='center'>$num</td><td align='left'$highlight><a href=userdetails.php?id=".$a['userid']."> <b><font color='#" . get_user_class_color($a['class']) . "'> " . htmlspecialchars($a['username']) . "</font></b>".get_user_icons($a)."</a>" ."</td>
    <td align='right'$highlight>" . mksize($a["uploaded"]) ."</td>
    <td align='right'$highlight>" . mksize($a["upspeed"]) . "/s" ."</td>
    <td align='right'$highlight>" . mksize($a["downloaded"]) ."</td>
    <td align='right'$highlight>" . mksize($a["downspeed"]) . "/s" ."</td>
    <td align='right'$highlight>" . $ratio ."</td>
    <td align='left'>" . get_date( $a['added'],'') . " (" .
...
if ($a["leechers"]){
    ...
    $a["name"] . "</b></a></td><td align='right'>" . number_format($a["times_completed"]) ."</td>
    <td align='right'>" . mksize($a["data"]) . "</td><td align='right'>" . number_format($a["seeders"]) ."</td>
    <td align='right'>" . number_format($a["leechers"]) . "</td><td align='right'>" . ($a["leechers"] + $a["seeders"]) ."</td>
...
if ($what == "Users")
    ...
    $htmlout .= "<tr><td align='center'>$num</td>
    <td align='left'>
    <table border='0' class='main' cellspacing='0' cellpadding='0'><tr><td class='embedded'>
    "."
    <img src=\"{$FMED['pic_base_url']}flag/{$a['flagpic']}\" alt='' /></td>
    <td class='embedded' style='padding-left: 5px'><b>$a[name]</b></td>"."</tr></table></td>
...
global $lang;
    ...
    $htmlout .= "<tr>
    <td class='colhead'>{$lang['common_rank']}</td>
    <td class='colhead'>{$lang['peers_uname']}</td>
    <td class='colhead'>{$lang['peers_ulrate']}</td>
    <td class='colhead'>{$lang['peers_dlrate']}</td></tr>";
    $n = 1;
    while ($arr = mysqli_fetch_assoc($res)){
    $highlight = $CURUSER["id"] == $arr["userid"] ? " bgcolor='#BBAF9B'" : "";
    $htmlout .= "<tr>
    <td$highlight>$n</td><td$highlight><a href='userdetails.php?id=" . $arr["userid"] . "'><b>" . $arr["username"] . "</b></a></td>
    <td$highlight>" . mksize($arr["uprate"]) . "/s</td>
    <td$highlight>" . mksize($arr["downrate"]) . "/s</td></tr>\n";
    ++$n;
}
    $htmlout .= end_table();
    $htmlout .= end_frame();
...
if (!$pu)
    $limit = 10;
if ($type == 1){
    $mainquery = "SELECT id as userid, username, class, donor, warned, added, uploaded, downloaded, uploaded / (".time()." - added) AS upspeed, downloaded / (".time()." - added) AS downspeed FROM users WHERE enabled = 'yes'";
if (!$limit || $limit > 250)
    $limit = 10;
if ($limit == 10 || $subtype == "ul"){
    $order = "uploaded DESC";
    $r = mysqli_query($GLOBALS["___mysqli_ston"], $mainquery . " ORDER BY $order " . " LIMIT $limit") or sqlerr();
...
if ($limit == 10 || $subtype == "bsh"){
    ...
    $r = mysqli_query($GLOBALS["___mysqli_ston"], $mainquery . $extrawhere . " ORDER BY $order " . " LIMIT $limit") or sqlerr();
...
if ($limit == 10 || $subtype == "act"){
    $r = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT t.*, (t.size * t.times_completed + SUM(p.downloaded)) AS data FROM torrents AS t LEFT JOIN peers AS p ON t.id = p.torrent WHERE p.seeder = 'no' GROUP BY t.id ORDER BY seeders + leechers DESC, seeders DESC, added ASC LIMIT $limit") or sqlerr();
...
if ($limit == 10 || $subtype == "sna"){
    $r = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT t.*, (t.size * t.times_completed + SUM(p.downloaded)) AS data FROM torrents AS t LEFT JOIN peers AS p ON t.id = p.torrent WHERE p.seeder = 'no' GROUP BY t.id ORDER BY times_completed DESC LIMIT $limit") or sqlerr();
...
if ($limit == 10 || $subtype == "mdt"){
    $r = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT t.*, (t.size * t.times_completed + SUM(p.downloaded)) AS data FROM torrents AS t LEFT JOIN peers AS p ON t.id = p.torrent WHERE p.seeder = 'no' AND leechers >= 5 AND times_completed > 0 GROUP BY t.id ORDER BY data DESC, added ASC LIMIT $limit") or sqlerr();
...
if ($limit == 10 || $subtype == "bse"){
    $r = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT t.*, (t.size * t.times_completed + SUM(p.downloaded)) AS data FROM torrents AS t LEFT JOIN peers AS p ON t.id = p.torrent WHERE p.seeder = 'no' AND seeders >= 5 GROUP BY t.id ORDER BY seeders / leechers DESC, seeders DESC, added ASC LIMIT $limit") or sqlerr();
...
if ($limit == 10 || $subtype == "wse"){
    $r = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT t.*, (t.size * t.times_completed + SUM(p.downloaded)) AS data FROM torrents AS t LEFT JOIN peers AS p ON t.id = p.torrent WHERE p.seeder = 'no' AND leechers >= 5 AND times_completed > 0 GROUP BY t.id ORDER BY seeders / leechers ASC, leechers DESC LIMIT $limit") or sqlerr();
...
if ($limit == 10 || $subtype == "us"){
    $r = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT name, flagpic, COUNT(users.country) as num FROM countries LEFT JOIN users ON users.country = countries.id GROUP BY name ORDER BY num DESC LIMIT $limit") or sqlerr();
...
if ($limit == 10 || $subtype == "ul"){
    $r = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT c.name, c.flagpic, sum(u.uploaded) AS ul FROM users AS u LEFT JOIN countries AS c ON u.country = c.id WHERE u.enabled = 'yes' GROUP BY c.name ORDER BY ul DESC LIMIT $limit") or sqlerr();
...
if ($limit == 10 || $subtype == "avg"){
    $r = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT c.name, c.flagpic, sum(u.uploaded)/count(u.id) AS ul_avg FROM users AS u LEFT JOIN countries AS c ON u.country = c.id WHERE u.enabled = 'yes' GROUP BY c.name HAVING sum(u.uploaded) > 1099511627776 AND count(u.id) >= 100 ORDER BY ul_avg DESC LIMIT $limit") or sqlerr();
...
if (!$limit || $limit > 250)
    $limit = 10;
if ($limit == 10 || $subtype == "ul"){
    $r = mysqli_query($GLOBALS["___mysqli_ston"],  "SELECT users.id AS userid, username, class, donor, warned, (peers.uploaded - peers.uploadoffset) / (last_action - started) AS uprate, IF(seeder = 'yes',(peers.downloaded - peers.downloadoffset)  / (finishedat - started),(peers.downloaded - peers.downloadoffset) / (last_action - started)) AS downrate FROM peers LEFT JOIN users ON peers.userid = users.id ORDER BY uprate DESC LIMIT $limit") or sqlerr();
...
if ($limit == 10 || $subtype == "dl"){
    $r = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT users.id AS userid, peers.id AS peerid, username, class, donor, warned, peers.uploaded, peers.downloaded,(peers.uploaded - peers.uploadoffset) / (last_action - started) AS uprate, IF(seeder = 'yes',(peers.downloaded - peers.downloadoffset)  / (finishedat - started),(peers.downloaded - peers.downloadoffset) / (last_action - started)) AS downrate FROM peers LEFT JOIN users ON peers.userid = users.id ORDER BY downrate DESC LIMIT $limit") or sqlerr();
